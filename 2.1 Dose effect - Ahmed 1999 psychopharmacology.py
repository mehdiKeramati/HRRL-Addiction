...
'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Reset Logging information of ShA   ------------------------------------------------------------ 
--------------------------------------------------------------------------------------------------------------------------------'''
def resetLogInfoShA():    

    for i in range(0,totalTrialsNum):
        nulDoingShA            [i] = 0
        inactiveLeverPressShA  [i] = 0
        activeLeverPressShA    [i] = 0
        internalStateShA       [i] = 0
        setpointShA            [i] = 0
        infusionShA            [i] = 0
...
def pretraining  (ratType):
    ...
    for trial in range(0,trialsNum):
        ...
        if ratType=='ShA':  
            ...
            print "Dose = %d mmg    ShA pre-training       trial: %d / %d" %(cocaine*5,trial+1,trialsNum)
        elif ratType=='LgA':  
            loggingLgA (trial,action,inState,setpointS,out)    
            print "Dose = %d mmg    LgA pre-training       trial: %d / %d" %(cocaine*5,trial+1,trialsNum)
...
def cocaineSeeking  (sessionNum , ratType):
    ...
    for trial in range(trialCount,trialCount+trialsNum):
        ...
        if ratType=='ShA':  
            loggingShA(trial,action,inState,setpointS,out)    
            print "Dose = %d mmg    ShA cocaine seeking    trial: %d / %d" %(cocaine*5,trial-trialCount+1,trialsNum)
        if ratType=='LgA':  
            loggingLgA(trial,action,inState,setpointS,out)    
            print "Dose = %d mmg    LgA cocaine seeking    trial: %d / %d" %(cocaine*5,trial-trialCount+1,trialsNum)
...
def homeCage (sessionNum, ratType):
    ...
    if ratType=='ShA':  
        ...
        print "                          animal rests in home cage" 
    elif ratType=='LgA':  
...
'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Wrap up all the logged data for ShA  ---------------------------------------------------------- 
--------------------------------------------------------------------------------------------------------------------------------'''
def loggingFinalizationShA():
...
'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Wrap up all the logged data for LgA  ---------------------------------------------------------- 
--------------------------------------------------------------------------------------------------------------------------------'''
def loggingFinalizationLgA():
...
infusionDoseShA = numpy.zeros([4],float)
infusionDoseLgA = numpy.zeros([4],float)
...
for animal in range(0,animalsNum):    
    ...
    for i in range(0,4):
        cocaine                 = baseDose * ( 2**i )            # Dose of self-administered drug
        setOutcome(0,1,0,cocaine)       # At state s, by doing action a and going to state s', we receive the outcome 
        ...
        loggingFinalizationShA()
        
        infusionPerHourShA = 0
        for j in range(trialsPerDay , trialsPerDay+trialsPerHour):
            infusionPerHourShA = infusionPerHourShA + infusionShA[j]        
        
        infusionDoseShA[i] = infusionPerHourShA
        ...
        loggingFinalizationLgA()
        
        infusionPerHourLgA = 0
        for j in range(trialsPerDay , trialsPerDay+trialsPerHour):
            infusionPerHourLgA = infusionPerHourLgA + infusionLgA[j]        
    
        infusionDoseLgA[i] = infusionPerHourLgA
...
S0 = ax1.plot(x,infusionDoseShA, '-o', ms=8, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )
S1 = ax1.plot(x,infusionDoseLgA , '-o', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
leg=fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.64,0.65))
leg.draw_frame(False)
pylab.yticks(pylab.arange(0, 241, 40))
pylab.ylim((0,280))

pylab.xlim((0,280))
tick_lcs = [31,62,94,125,156,188,219,250]
tick_lbs = ['.03','.06',' ','.125',' ',' ',' ','.250']
...
ax1.set_ylabel('# cocaine infusion / h')
ax1.set_xlabel('Cocaine dose (mg/infusion)')
ax1.set_title('Post-escelation')
fig1.savefig('infusionDoseCurve.eps', format='eps')

'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Plotting 1  ------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------------------------'''    

consumptionDoseShA = numpy.zeros([4],float)
consumptionDoseLgA = numpy.zeros([4],float)

for j in range(0 , 4):
    cocaine                 = 0.03125 * (baseDose/6.25) * ( 2**j )            # Dose of self-administered drug
    consumptionDoseShA [j] =  infusionDoseShA[j] * cocaine
    consumptionDoseLgA [j] =  infusionDoseLgA[j] * cocaine
...
fig1.subplots_adjust(bottom=0.2)
ax1 = fig1.add_subplot(111)
S0 = ax1.plot(x,consumptionDoseShA, '-o', ms=8, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )
S1 = ax1.plot(x,consumptionDoseLgA , '-o', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
leg=fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.24,0.65))
leg.draw_frame(False)
pylab.ylim((0,13))
pylab.yticks(pylab.arange(0, 13, 2))
...
ax1.set_xlabel('Cocaine dose (mg/infusion)')
ax1.set_title('Post-escelation')
fig1.savefig('consumptionDoseCurve.eps', format='eps')
