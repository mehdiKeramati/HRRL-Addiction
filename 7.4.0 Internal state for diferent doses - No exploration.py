...
dosesNum = 25
infusionPerDose = numpy.zeros([dosesNum],float)

for i in range(0,dosesNum):
    
    print "Dose num: %d / %d           Dose = %f  " %(i+1,dosesNum,( ( 11.5 + 1.3**(i) ) * 0.005 ))
    ...
    infusionPerDose[i] = infRate/seekingHoursLgA
...
#infusionPerDose[9] = 0
S1 = ax1.plot(infusionPerDose , 'o', ms=8, markeredgewidth =1.5, alpha=1, mfc='black',linewidth = 2 , color='red' )

def fitFuncFree(x, a0, a1, a2, a3, a4, a5):
    z = (x - a1) / a2
...
parameters, covariance = curve_fit(fitFuncGuassian, x, infusionPerDose)

fitted = fitFuncGuassian(x,parameters[0],parameters[1],parameters[2])
#S2 = ax1.plot(fitted , '-', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='red' )


pylab.yticks(pylab.arange(0, 36, 5))
pylab.ylim((-2,37))
...
consumptionDoseLgA = numpy.zeros([dosesNum],float)

for j in range(0 , dosesNum):
    cocaine                 = ( 11.5 + (1.3)**j ) * 0.005            # Dose of self-administered drug
    consumptionDoseLgA [j] =  infusionPerDose[j] * cocaine
...
S1 = ax1.plot(consumptionDoseLgA , 'o', ms=8, markeredgewidth =1.5, alpha=1, mfc='black',linewidth = 2 , color='red' )
...
x = numpy.arange(0, dosesNum, 1)
parameters, covariance = curve_fit(fitFuncSigmoid, x[0:18], consumptionDoseLgA[0:18])

fitted = fitFuncSigmoid(x[0:18],parameters[0],parameters[1],parameters[2])
S2 = ax1.plot(x[0:18],fitted , '-', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='red' )


pylab.ylim((-.5,5))
pylab.yticks(pylab.arange(0, 5, 1))
