...
#    Assuming that the animals know the energy cost (fatigue) of pressing a lever 
    ...
    estimatedOutcome [0][2][1]     = cocaine
...
def plotSetpoint():
    ...
    S0 = ax1.plot(setpointShA [0 : trialsPerDay*(sessionsNum+0)] , linewidth = 1.5 , color='black' )
    S1 = ax1.plot(setpointLgA [0 : trialsPerDay*(sessionsNum+0)] , linewidth = 2.5 , color='black' )
   
#    box = ax1.get_position()
#    ax1.set_position([box.x0, box.y0, box.width * 0.8, box.height])
#    leg = ax1.legend((S1, S0), ('LgA',' ShA'), loc='center left' , bbox_to_anchor=(1, 0.5))
...
def plotInfusionPerSession():
    ...
    for i in range(0,sessionsNum):
        for j in range(0 + i*(trialsPerDay),0 + i*(trialsPerDay)+seekingTrialsNumShA):
            infusionPerSessionShA[i+1] = infusionPerSessionShA[i+1] + infusionShA[j]

    for i in range(0,sessionsNum):
        for j in range(0 + i*(trialsPerDay),0 + i*(trialsPerDay)+seekingTrialsNumLgA):
...
def plotInfusionPerFirstHour():
    ...
    for i in range(0,sessionsNum):
        for j in range(0 + i*(trialsPerDay),0 + i*(trialsPerDay)+trialsPerHour):
...
pretrainingHours    = 1
sessionsNum         = 9                                  # Number of sessions of cocain seeking, followed by rest in home-cage
seekingHoursShA     = 3            
seekingHoursLgA     = 3            
extinctionHours     = 0
