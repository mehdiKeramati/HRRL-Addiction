...
def plotInternalStateLastSession():
    ...
    S0 = ax1.plot(internalStateShA [trialsPerDay*sessionsNum - 50 : trialsPerDay*sessionsNum + 500 ] , linewidth = 1.5 , color='black' )
    S1 = ax1.plot(internalStateLgA [trialsPerDay*sessionsNum - 50 : trialsPerDay*sessionsNum + 500 ] , linewidth = 2.5 , color='black' )
  
    leg = fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.64,0.68))
    leg.draw_frame(False)

    pylab.ylim((-10 , 350))
    pylab.yticks(pylab.arange(0, 301, 100))
    pylab.xlim((0,551))
...
def plotSetpoint():
    ...
    S0 = ax1.plot(setpointShA [trialsPerDay : trialsPerDay*(sessionsNum+1)] , linewidth = 1.5 , color='black' )
    S1 = ax1.plot(setpointLgA [trialsPerDay : trialsPerDay*(sessionsNum+1)] , linewidth = 2.5 , color='black' )
  
    leg = fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.16,0.65))
    leg.draw_frame(False)
...
def plotSetpoint234():
    ...
    S0 = ax1.plot(setpointShA [ trialsPerDay*2 - trialsPerHour*2 : trialsPerDay*5 + trialsPerHour*2] , linewidth = 1.5 , color='black' )
    S1 = ax1.plot(setpointLgA [ trialsPerDay*2 - trialsPerHour*2 : trialsPerDay*5 + trialsPerHour*2] , linewidth = 2.5 , color='black' )
  
    leg = fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.16,0.65))
    leg.draw_frame(False)
      
    pylab.yticks(pylab.arange(100, 121, 5))
    pylab.ylim((98,122))
    ...
    for i in range ( 2 , 5 ):
        p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*(i-2) , trialsPerHour*2 + trialsPerDay*(i-2) + trialsPerHour*seekingHoursShA , ymin=0 , ymax=0.2 , facecolor='1',edgecolor='none', alpha=0.5)        
        p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*(i-2) + trialsPerHour*seekingHoursShA , trialsPerHour*2 + trialsPerDay*(i-1), ymin=0 , ymax=0.2 , facecolor='0.750',edgecolor='none', alpha=0.5)        
        p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*(i-2) , trialsPerHour*2 + trialsPerDay*(i-2) + trialsPerHour*seekingHoursLgA , ymin=0.2 , ymax=1 , facecolor='1',edgecolor='none', alpha=0.5)        
        p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*(i-2) + trialsPerHour*seekingHoursLgA , trialsPerHour*2 + trialsPerDay*(i-1), ymin=0.2 , ymax=1 , facecolor='0.750',edgecolor='none', alpha=0.5)        

    p = pylab.axvspan( 0 , trialsPerHour*2 , ymin=0 , ymax=1 , facecolor='0.750',edgecolor='none', alpha=0.5)        
    p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*3  , trialsPerHour*2 + trialsPerDay*3 + trialsPerHour , ymin=0 , ymax=0.2 , facecolor='1',edgecolor='none', alpha=0.5)        
    p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*3 + trialsPerHour ,trialsPerHour*2 + trialsPerDay*3 + trialsPerHour*2 , ymin=0 , ymax=0.2 , facecolor='0.570',edgecolor='none', alpha=0.5)        
    p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*3 , trialsPerHour*2 + trialsPerDay*3 + trialsPerHour*2 , ymin=0.2 , ymax=1 , facecolor='1',edgecolor='none', alpha=0.5)        
...
def plotInfusionPerFirstHour():
    ...
    leg=fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.20,0.68))
    leg.draw_frame(False)

#    pylab.yticks(pylab.arange(0, 31, 5))
    pylab.ylim((0,maxInjPerHour))
    pylab.xlim((0,sessionsNum+1))
    pylab.xticks(pylab.arange(1, sessionsNum+1, 4))
...
'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Plot the infusions per hour for the first and the last three sessions in the LgA --------------
--------------------------------------------------------------------------------------------------------------------------------'''
def plotInfusionPerHourLgA():
    
    infusionPerHourSession1  = numpy.zeros( [6] , float)
    infusionPerHourSession2  = numpy.zeros( [6] , float)
    infusionPerHourSession3  = numpy.zeros( [6] , float)
    infusionPerHourSession_1 = numpy.zeros( [6] , float)
    infusionPerHourSession_2 = numpy.zeros( [6] , float)
    infusionPerHourSession_3 = numpy.zeros( [6] , float)

    x = numpy.arange(1, 7, 1)
    
    for i in range(0,6):
        for j in range(trialsPerDay + 0*trialsPerDay + i*trialsPerHour , trialsPerDay + 0*trialsPerDay + (i+1)*trialsPerHour):
            infusionPerHourSession1[i] = infusionPerHourSession1[i] + infusionLgA[j]
    for i in range(0,6):
        for j in range(trialsPerDay + 1*trialsPerDay + i*trialsPerHour , trialsPerDay + 1*trialsPerDay + (i+1)*trialsPerHour):
            infusionPerHourSession2[i] = infusionPerHourSession2[i] + infusionLgA[j]
    for i in range(0,6):
        for j in range(trialsPerDay + 2*trialsPerDay + i*trialsPerHour , trialsPerDay + 2*trialsPerDay + (i+1)*trialsPerHour):
            infusionPerHourSession3[i] = infusionPerHourSession3[i] + infusionLgA[j]

    for i in range(0,6):
        for j in range(trialsPerDay + (sessionsNum-1)*trialsPerDay + i*trialsPerHour , trialsPerDay + (sessionsNum-1)*trialsPerDay + (i+1)*trialsPerHour):
            infusionPerHourSession_1[i] = infusionPerHourSession_1[i] + infusionLgA[j]
    for i in range(0,6):
        for j in range(trialsPerDay + (sessionsNum-2)*trialsPerDay + i*trialsPerHour , trialsPerDay + (sessionsNum-2)*trialsPerDay + (i+1)*trialsPerHour):
            infusionPerHourSession_2[i] = infusionPerHourSession_2[i] + infusionLgA[j]
    for i in range(0,6):
        for j in range(trialsPerDay + (sessionsNum-3)*trialsPerDay + i*trialsPerHour , trialsPerDay + (sessionsNum-3)*trialsPerDay + (i+1)*trialsPerHour):
            infusionPerHourSession_3[i] = infusionPerHourSession_3[i] + infusionLgA[j]

    infusionPerHourSession123   = numpy.zeros( [6] , float)
    infusionPerHourSession_123  = numpy.zeros( [6] , float)

    infusionPerHourSession123   = (infusionPerHourSession1 + infusionPerHourSession2 + infusionPerHourSession3 ) / 3 
    infusionPerHourSession_123  = (infusionPerHourSession_1+ infusionPerHourSession_2+ infusionPerHourSession_3) / 3
        
    fig1 = pylab.figure( figsize=(5,3.5) )
    fig1.subplots_adjust(bottom=0.2)
    fig1.subplots_adjust(left=0.16)
    ax1 = fig1.add_subplot(111)
    S0 = ax1.plot(x,infusionPerHourSession123 , '-o', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
    S1 = ax1.plot(x,infusionPerHourSession_123, '-o', ms=8, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )

    leg=fig1.legend((S0, S1), ('First 3 sessions','Last 3 sessions'), loc = (0.35,0.70))
    leg.draw_frame(False)
    
#    pylab.yticks(pylab.arange(0, 31, 5))
    pylab.ylim((0,maxInjPerHour))
    pylab.xlim((0,7))
    pylab.xticks(pylab.arange(1, 7, 1))
    
    for line in ax1.get_xticklines() + ax1.get_yticklines():
        line.set_markeredgewidth(2)
        line.set_markersize(5)

    ax1.set_ylabel('Infusions / hour')
    ax1.set_xlabel('Hour')
    ax1.set_title(label)
    fig1.savefig('infusionPerHourLgA.eps', format='eps')
...
--------------------------------------------------------------------------------------------------------------------------------'''
def plotInfusionPer10MinShA():
    
    infusionFirstSession = numpy.zeros( [6] , float)
    infusionLastSession  = numpy.zeros( [6] , float)
    x = numpy.arange(10, 61, 10)
    
    for i in range(0,6):
        for j in range(trialsPerDay + i*(trialsPerBlock), trialsPerDay + (i+1)*trialsPerBlock):
            infusionFirstSession[i] = infusionFirstSession[i] + infusionShA[j]
    for i in range(0,6):
        for j in range(trialsPerDay*sessionsNum + i*(trialsPerBlock), trialsPerDay*sessionsNum + (i+1)*trialsPerBlock):
            infusionLastSession[i] = infusionLastSession[i] + infusionShA[j]
        
    fig1 = pylab.figure( figsize=(5,3.5) )
    fig1.subplots_adjust(bottom=0.2)
    fig1.subplots_adjust(left=0.16)
    ax1 = fig1.add_subplot(111)
    S0 = ax1.plot(x,infusionFirstSession, '-o', ms=8, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )
    S1 = ax1.plot(x,infusionLastSession , '-o', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )

    leg=fig1.legend((S1, S0), ('Last session','First session'), loc = (0.45,0.70))
    leg.draw_frame(False)
    
#    pylab.yticks(pylab.arange(1, 8, 1))

    
# -------------------------------------- JUST FOR FINDING THE RANGE OF Y AXIS    
    infusionFirstSessionL = numpy.zeros( [6] , float)
    infusionLastSessionL  = numpy.zeros( [6] , float)
    
    for i in range(0,6):
        for j in range(trialsPerDay + i*(trialsPerBlock), trialsPerDay + (i+1)*trialsPerBlock):
            infusionFirstSessionL[i] = infusionFirstSessionL[i] + infusionLgA[j]
    for i in range(0,6):
        for j in range(trialsPerDay*sessionsNum + i*(trialsPerBlock), trialsPerDay*sessionsNum + (i+1)*trialsPerBlock):
            infusionLastSessionL[i] = infusionLastSessionL[i] + infusionLgA[j]

    max = 0
    for i in range(0,6):
        if max < infusionLastSessionL[i]:
            max = infusionLastSessionL[i]    
        if max < infusionFirstSessionL[i]:
            max = infusionFirstSessionL[i]    
# -------------------------------------- End
    
    pylab.ylim((-0.17,max + max/6 ))
    pylab.xlim((0,70))
    ...
    ax1.set_xlabel('Time (min)')
    ax1.set_title('ShA animal')
    fig1.savefig('infusionPer10MinShA.eps', format='eps')

    return

'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Plot the infusions per 10 minutes for the Long-Access group ----------------------------------- 
--------------------------------------------------------------------------------------------------------------------------------'''
def plotInfusionPer10MinLgA():
    ...
    for i in range(0,6):
        for j in range(trialsPerDay + i*(trialsPerBlock), trialsPerDay + (i+1)*trialsPerBlock):
            infusionFirstSession[i] = infusionFirstSession[i] + infusionLgA[j]
    for i in range(0,6):
        for j in range(trialsPerDay*sessionsNum + i*(trialsPerBlock), trialsPerDay*sessionsNum + (i+1)*trialsPerBlock):
            infusionLastSession[i] = infusionLastSession[i] + infusionLgA[j]

    max = 0
    for i in range(0,6):
        if max < infusionLastSession[i]:
            max = infusionLastSession[i]
...
#    pylab.yticks(pylab.arange(1, 8, 1))
    ...
    ax1.set_ylabel('Infusions / 10 min')
    ax1.set_xlabel('Time (min)')
    ax1.set_title('LgA animal')
    fig1.savefig('infusionPer10MinLgA.eps', format='eps')
...
# --------------------------------------- Compute III For Long-Access
    ...
    infusionsNumLgA = len(iiiLgA)
    if infusionsNumLgA > 30 :
        infusionsNumLgA = 30
    xLgA = numpy.arange(1, infusionsNumLgA+1, 1)
...
# --------------------------------------- Compute III For Short-Access
    ...
    if infusionsNumShA > 30 :
        infusionsNumShA = 30        
    xShA = numpy.arange(1, infusionsNumShA+1, 1)
    

    mini = infusionsNumShA
    iiimax = 0
    ...
    S1 = ax1.plot(xLgA,iiiLgA[0:infusionsNumLgA], '-o', ms=5, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )
    
    leg=fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.65,0.70))
    leg.draw_frame(False)
    
    pylab.ylim((0,iiimax+20))
    pylab.xlim((0 , infusionsNumLgA + 1))
...
def plotting():
    ...
    plotInfusionPerHourLgA()
    plotInfusionPer10MinShA()
    plotInfusionPer10MinLgA()
...
cocaine         = 12.5             # Dose of self-administered drug = 0.0625 mg
maxInjPerSess   = 800            # the maximum number of injections/session that the LgA group will reach 300
maxInjPerHour   = 160
maxInternalState= 250            # the maximum value that the internal state reaches for the LgA group
label           = '0.062 mg/infusion'

cocaine         = 25             # Dose of self-administered drug = 0.125 mg
maxInjPerSess   = 400            # the maximum number of injections/session that the LgA group will reach 300
maxInjPerHour   = 100
maxInternalState= 250            # the maximum value that the internal state reaches for the LgA group
label           = '0.125 mg/infusion'


cocaine         = 50             # Dose of self-administered drug = 0.250 mg
maxInjPerSess   = 200            # the maximum number of injections/session that the LgA group will reach 150
maxInjPerHour   = 60
maxInternalState= 250            # the maximum value that the internal state reaches for the LgA group
label           = '0.250 mg/infusion'
'''

cocaine         = 100            # Dose of self-administered drug = 0.5   mg
maxInjPerSess   = 100            # the maximum number of injections/session that the LgA group will reach 75
maxInjPerHour   = 35
maxInternalState= 250            # the maximum value that the internal state reaches for the LgA group
label           = '0.5 mg/infusion'
