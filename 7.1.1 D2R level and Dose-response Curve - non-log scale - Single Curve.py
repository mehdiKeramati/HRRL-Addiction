...
t = numpy.zeros([dosesNum],float)

for i in range(0,dosesNum):
    t[i]         = ( 11.5 + (1.3)**i ) * 0.005             # Dose of self-administered drug

S1 = ax1.plot(t,infusionPerDose , 'o', ms=8, markeredgewidth =1.5, alpha=1, mfc='black',linewidth = 2 , color='red' )


def fitFuncFree(x, a0, a1, a2, a3):
    y = a0 * numpy.exp(-a1*x) - a2 * numpy.exp(-a3*x) 
    return y
 

pylab.yticks(pylab.arange(0, 21, 5))
pylab.ylim((-3,25))

pylab.xticks(pylab.arange(0, 3.1, .5))
pylab.xlim((-0.15,3.15))
...
S1 = ax1.plot(t,consumptionDoseLgA , 'o', ms=8, markeredgewidth =1.5, alpha=1, mfc='black',linewidth = 2 , color='red' )
