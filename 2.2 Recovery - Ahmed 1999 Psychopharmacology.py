...
def homeCage (sessionNum, ratType):
    ...
    elif ratType=='wholeDayShA':  
        trialsNum = trialsPerDay    
        print "ShA rat number: %d / %d     Session Number: %d / %d                          animal rests in home cage" %(animal+1,animalsNum,sessionNum+1,sessionsNum)
    elif ratType=='wholeDayLgA':  
        trialsNum = trialsPerDay    
        print "LgA rat number: %d / %d     Session Number: %d / %d                          animal rests in home cage" %(animal+1,animalsNum,sessionNum+1,sessionsNum)
...
def plotSetpoint():
    ...
    ax1.set_xlabel('Post-escalation Day')
    ax1.set_title('')
...
def plotInfusionPerSession():
    ...
    infusionPerSessionShA = numpy.zeros( [24] , float)
    infusionPerSessionLgA = numpy.zeros( [24] , float)
    x = numpy.arange(1, 24, 1)
    x[20] = 25
    x[21] = 30
    x[22] = 34
    ...
    i=25
    for j in range(trialsPerDay + i*(trialsPerDay),trialsPerDay + i*(trialsPerDay)+seekingTrialsNumShA):
        infusionPerSessionShA[21] = infusionPerSessionShA[21] + infusionShA[j]
    i=30
    for j in range(trialsPerDay + i*(trialsPerDay),trialsPerDay + i*(trialsPerDay)+seekingTrialsNumShA):
        infusionPerSessionShA[22] = infusionPerSessionShA[22] + infusionShA[j]
    i=34
    for j in range(trialsPerDay + i*(trialsPerDay),trialsPerDay + i*(trialsPerDay)+seekingTrialsNumShA):
        infusionPerSessionShA[23] = infusionPerSessionShA[23] + infusionShA[j]
    ...
    i=25
    for j in range(trialsPerDay + i*(trialsPerDay),trialsPerDay + i*(trialsPerDay)+seekingTrialsNumLgA):
        infusionPerSessionLgA[21] = infusionPerSessionLgA[21] + infusionLgA[j]
    i=30
    for j in range(trialsPerDay + i*(trialsPerDay),trialsPerDay + i*(trialsPerDay)+seekingTrialsNumLgA):
        infusionPerSessionLgA[22] = infusionPerSessionLgA[22] + infusionLgA[j]
    i=34
    for j in range(trialsPerDay + i*(trialsPerDay),trialsPerDay + i*(trialsPerDay)+seekingTrialsNumLgA):
        infusionPerSessionLgA[23] = infusionPerSessionLgA[22] + infusionLgA[j]

        
    fig1 = pylab.figure( figsize=(5,3.5) )
    fig1.subplots_adjust(bottom=0.2)
    ax1 = fig1.add_subplot(111)
    S0 = ax1.plot(x,infusionPerSessionShA[1:24] , '-o', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
    S1 = ax1.plot(x,infusionPerSessionLgA[1:24] , '-o', ms=8, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )
    ...
    pylab.xticks(pylab.arange(0, sessionsNum+1, 5))
    
    for line in ax1.get_xticklines() + ax1.get_yticklines():
        line.set_markeredgewidth(2)
        line.set_markersize(5)

    ax1.set_ylabel('# Infusions / h')
    ax1.set_xlabel('Post-escalation Day')
    ax1.set_title(' ')
...
pretrainingHours    = 1
sessionsNum         = 35                                  # Number of sessions of cocain seeking, followed by rest in home-cage
seekingHoursShA     = 1            
seekingHoursLgA     = 1            
extinctionHours     = 0
...
for animal in range(0,animalsNum):
    ...
    for session in range(0,20):
        cocaineSeeking        (  session , 'ShA'        )
        homeCage              (  session , 'ShA'        ) 
    for session in range(20,25):
        homeCage              (  session , 'wholeDayShA') 
    cocaineSeeking        (  25          , 'ShA'        )
    homeCage              (  25          , 'ShA'        ) 
    for session in range(26,30):
        homeCage              (  session , 'wholeDayShA') 
    cocaineSeeking        (  30          , 'ShA'        )
    homeCage              (  30          , 'ShA'        ) 
    for session in range(31,34):
        homeCage              (  session , 'wholeDayShA') 
    cocaineSeeking        (  34          , 'ShA'        )
    homeCage              (  34          , 'ShA'        ) 
    ...
    for session in range(20,25):
        homeCage              (  session , 'wholeDayLgA') 
    cocaineSeeking        (  25          , 'LgA'        )
    homeCage              (  25          , 'LgA'        ) 
    for session in range(26,30):
        homeCage              (  session , 'wholeDayLgA') 
    cocaineSeeking        (  30          , 'LgA'        )
    homeCage              (  30          , 'LgA'        ) 
    for session in range(31,34):
        homeCage              (  session , 'wholeDayLgA') 
    cocaineSeeking        (  34          , 'LgA'        )
    homeCage              (  34          , 'LgA'        ) 
