...
def cocaineSeeking  (sessionNum , ratType):
    ...
    for trial in range(trialCount,trialCount+trialsNum):
        ...
        if ratType=='ShA':  
            loggingShA(trial,exState,action,inState,setpointS,out)    
        ...
        if ratType=='LgA':  
            loggingLgA(trial,exState,action,inState,setpointS,out)    
...
#        print "LgA rat number: %d / %d     Session Number: %d / %d                          animal rests in home cage" %(animal+1,animalsNum,sessionNum+1,sessionsNum)
    ...
    for trial in range(trialCount,trialCount+trialsNum):
        ...
        if ratType=='ShA':  
            loggingShA(trial,0,0,inState,setpointS,0)    
        elif ratType=='LgA':  
            loggingLgA(trial,0,0,inState,setpointS,0)    
        elif ratType=='afterPretrainingShA':  
            loggingShA(trial,0,0,inState,setpointS,0)    
        elif ratType=='afterPretrainingLgA':  
            loggingLgA(trial,0,0,inState,setpointS,0)    
...
--------------------------------------------------------------------------------------------------------------------------------'''
def loggingLgA(trial,state,action,inState,setpointS,coca):
    ...
    if state==0 and action==2:
        foodTakingLgA[trial]   = foodTakingLgA[trial] +1
    return
...
def loggingFinalization():
    ...
    for trial in range(0,totalTrialsNum):
        ...
        foodTakingLgA[trial]           = foodTakingLgA[trial]/animalsNum 
...
foodTakingLgA          = numpy.zeros( [totalTrialsNum] , float)
...
foodPerDoseHigh     = numpy.zeros([dosesNum],float)
foodPerDoseLow      = numpy.zeros([dosesNum],float)
...
for i in range(0,dosesNum):
    ...
    cocaine         = 13 + (1.24)**i             # Dose of self-administered drug
    setOutcome(0,1,1,cocaine)              # At state s, by doing action a and going to state s', we receive the outcome 
    initializeAnimal        (   )
    resetLogInfoLgA       (                         )
    ...
    for j in range(0,seekingTrialsNumLgA):
        ...
        foodRate = foodRate + foodTakingLgA[j]    
    infusionPerDoseHigh[i] = infRate/seekingHoursLgA
    foodPerDoseHigh[i]     = foodRate/seekingHoursLgA
    ...
    foodPerDoseLow[i]     = foodRate/seekingHoursLgA
    

'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Plotting 1: Cocaine seeking / hour  -----------------------------------------------------------
--------------------------------------------------------------------------------------------------------------------------------'''    
...
S0 = ax1.plot(infusionPerDoseHigh , 'o-', ms=8, markeredgewidth =1.5, alpha=1, mfc='black',linewidth = 2 , color='black' )
S1 = ax1.plot(infusionPerDoseLow  , 'o-', ms=8, markeredgewidth =1.5, alpha=1, mfc='white',linewidth = 2 , color='black' )
...
leg=fig1.legend((S1, S0), ('D2R-Low','D2R-High'), loc = (0.18,0.68))
leg.draw_frame(False)

pylab.yticks(pylab.arange(0, 15, 3))
pylab.ylim((-1,15))

pylab.xlim((-1,dosesNum))
tick_lcs = [0,3,6,9,12]
tick_lbs = numpy.zeros( [5] , float)
for i in range(0,5): 
    cocaine         = 13 + (1.24)**(i*3)
    tick_lbs[i] = '%.2f' %(  cocaine * 0.005 )
...
ax1.set_ylabel('Cocaine response / h')
ax1.set_xlabel('Cocaine dose (mg/infusion)')
#ax1.set_title('Post-escelation')
fig1.savefig('CocaineResponses.eps', format='eps')

'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Plotting 2: Food seeking / hour  --------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------------------------'''    
...
S0 = ax1.plot(foodPerDoseHigh , 'o-', ms=8, markeredgewidth =1.5, alpha=1, mfc='black',linewidth = 2 , color='black' )
S1 = ax1.plot(foodPerDoseLow  , 'o-', ms=8, markeredgewidth =1.5, alpha=1, mfc='white',linewidth = 2 , color='black' )


def fitFuncGuassian(x, a0, a1, a2, a3):
    y = -a0 * numpy.exp( -a1* ( (a2-x)**2 ) ) + a3
    return y
 
#x = numpy.arange(0, dosesNum, 1)
#parameters, covariance = curve_fit(fitFuncGuassian, x, infusionPerDoseHigh)
#fitted = fitFuncGuassian(x,parameters[0],parameters[1],parameters[2],parameters[3])
#S2 = ax1.plot(fitted , '-', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
#
#parameters, covariance = curve_fit(fitFuncGuassian, x, infusionPerDoseLow)
#fitted = fitFuncGuassian(x,parameters[0],parameters[1],parameters[2],parameters[3])
...
leg=fig1.legend((S1, S0), ('D2R-Low','D2R-High'), loc = (0.18,0.25))
leg.draw_frame(False)

pylab.yticks(pylab.arange(10, 28, 4))
pylab.ylim((8,28))
...
ax1.set_ylabel('Food response / h')
ax1.set_xlabel('Cocaine dose (mg/infusion)')
#ax1.set_title('Post-escelation')
fig1.savefig('FoodResponses.eps', format='eps')


'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Plotting 3: Cocaine seeking Percentage  -------------------------------------------------------
--------------------------------------------------------------------------------------------------------------------------------'''    
 
infusionPercentPerDoseHigh = numpy.zeros([dosesNum],float)
infusionPercentPerDoseLow  = numpy.zeros([dosesNum],float)
foodPerPercentDoseHigh     = numpy.zeros([dosesNum],float)
foodPerPercentDoseLow      = numpy.zeros([dosesNum],float)

infusionPercentPerDoseHigh = 100 * infusionPerDoseHigh/(infusionPerDoseHigh + foodPerDoseHigh)
infusionPercentPerDoseLow  = 100 * infusionPerDoseLow/(infusionPerDoseLow  + foodPerDoseLow )
foodPerPercentDoseHigh     = 100 * foodPerDoseHigh    /(infusionPerDoseHigh + foodPerDoseHigh)
foodPerPercentDoseLow      = 100 * foodPerDoseLow    /(infusionPerDoseLow  + foodPerDoseLow )
...
S0 = ax1.plot(infusionPercentPerDoseHigh , 'o-', ms=8, markeredgewidth =1.5, alpha=1, mfc='black',linewidth = 2 , color='black' )
S1 = ax1.plot(infusionPercentPerDoseLow  , 'o-', ms=8, markeredgewidth =1.5, alpha=1, mfc='white',linewidth = 2 , color='black' )
...
#x = numpy.arange(0, dosesNum, 1)
#parameters, covariance = curve_fit(fitFuncGuassian, x, infusionPercentPerDoseHigh)
#fitted = fitFuncGuassian(x,parameters[0],parameters[1],parameters[2])
#S2 = ax1.plot(fitted , '-', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
#
#parameters, covariance = curve_fit(fitFuncGuassian, x, infusionPercentPerDoseLow )
#fitted = fitFuncGuassian(x,parameters[0],parameters[1],parameters[2])
#S3 = ax1.plot(fitted , '-', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
...
pylab.yticks(pylab.arange(0, 61, 10))
pylab.ylim((-5,68))
...
ax1.set_ylabel('% Cocaine response')
ax1.set_xlabel('Cocaine dose (mg/infusion)')
#ax1.set_title('Post-escelation')
fig1.savefig('CocaineResponsePercent.eps', format='eps')



'''--------------------------------------------------------------------------------------------------------------------------------
---------------------------------   Plotting 4: Food seeking Percent   ------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------------------------'''    
...
S0 = ax1.plot(foodPerPercentDoseHigh , 'o-', ms=8, markeredgewidth =1.5, alpha=1, mfc='black',linewidth = 2 , color='black' )
S1 = ax1.plot(foodPerPercentDoseLow  , 'o-', ms=8, markeredgewidth =1.5, alpha=1, mfc='white',linewidth = 2 , color='black' )
...
pylab.yticks(pylab.arange(30, 101, 10))
pylab.ylim((30,105))
...
ax1.set_ylabel('% Food response')
ax1.set_xlabel('Cocaine dose (mg/infusion)')
#ax1.set_title('Post-escelation')
fig1.savefig('FoodResponsePercent.eps', format='eps')
