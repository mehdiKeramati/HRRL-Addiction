...
def plotInternalStateLastSession():
    ...
    leg = fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.35,0.25))
    leg.draw_frame(False)
...
def plotSetpoint():
    ...
    leg = fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.20,0.65))
    leg.draw_frame(False)
...
def plotSetpoint234():
    ...
    for i in range ( 2 , 5 ):
        p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*(i-2) , trialsPerHour*2 + trialsPerDay*(i-2) + trialsPerHour*seekingHoursShA , ymin=0 , ymax=0.17 , facecolor='1',edgecolor='none', alpha=0.5)        
        p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*(i-2) + trialsPerHour*seekingHoursShA , trialsPerHour*2 + trialsPerDay*(i-1), ymin=0 , ymax=0.17 , facecolor='0.750',edgecolor='none', alpha=0.5)        
        p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*(i-2) , trialsPerHour*2 + trialsPerDay*(i-2) + trialsPerHour*seekingHoursLgA , ymin=0.17 , ymax=1 , facecolor='1',edgecolor='none', alpha=0.5)        
        p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*(i-2) + trialsPerHour*seekingHoursLgA , trialsPerHour*2 + trialsPerDay*(i-1), ymin=0.17 , ymax=1 , facecolor='0.750',edgecolor='none', alpha=0.5)        

    p = pylab.axvspan( 0 , trialsPerHour*2 , ymin=0 , ymax=1 , facecolor='0.750',edgecolor='none', alpha=0.5)        
    p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*3  , trialsPerHour*2 + trialsPerDay*3 + trialsPerHour , ymin=0 , ymax=0.17 , facecolor='1',edgecolor='none', alpha=0.5)        
    p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*3 + trialsPerHour ,trialsPerHour*2 + trialsPerDay*3 + trialsPerHour*2 , ymin=0 , ymax=0.17 , facecolor='0.570',edgecolor='none', alpha=0.5)        
    p = pylab.axvspan( trialsPerHour*2 + trialsPerDay*3 , trialsPerHour*2 + trialsPerDay*3 + trialsPerHour*2 , ymin=0.17 , ymax=1 , facecolor='1',edgecolor='none', alpha=0.5)        
...
#---------------------------------ShA rats
    ...
    fig1.subplots_adjust(left=0.16)
    ax1 = fig1.add_subplot(111)
    S0 = ax1.plot(infusionShA , linewidth = 2 , color='black' )
#    pylab.yticks(pylab.arange(0, 1.01, 0.2))
    pylab.ylim((0,2))
    pylab.xlim((0,totalTrialsNum))
#    pylab.xticks(pylab.arange(0, totalTrialsNum+1, trialsPerBlock))
...
#        line.set_markersize(25)
    ...
    ax1.set_xlabel('Time (min)')
    ax1.set_title('ShA animals')
    fig1.savefig('infusionShA.eps', format='eps')
...
def plotInfusionPerFirstHour():
    ...
    leg=fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.64,0.28))
    leg.draw_frame(False)
    
    pylab.yticks(pylab.arange(5, 26, 5))
    pylab.ylim((0,30))
    pylab.xlim((0,sessionsNum+1))
    pylab.xticks(pylab.arange(1, sessionsNum+1, 1))
...
def plotInfusionPer10MinShA():
    ...
    leg=fig1.legend((S1, S0), ('Last session','First session'), loc = (0.46,0.68))
    leg.draw_frame(False)
...
#--------------------------------------- Compute III For Short-Access
    ...
    leg=fig1.legend((S1, S0), ('LgA','ShA'), loc = (0.58,0.26))
    leg.draw_frame(False)
...
pretrainingHours    = 1
sessionsNum         = 12                                  # Number of sessions of cocain seeking, followed by rest in home-cage
seekingHoursShA     = 1            
seekingHoursLgA     = 6            
extinctionHours     = 0
