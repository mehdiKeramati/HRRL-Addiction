...
def extinction  ( sessionNum,trialsNum , ratsType):
    ...
    for trial in range(trialCount,trialCount+trialsNum):
        ...
        if ratsType == 'ShA':
            loggingShA(trial,action,inState,setpointS,out)   
            print "ShA rat number: %d / %d     Extinction session %d / %d       trial: %d / %d      Extinction of cocaine seeking" %(animal+1,animalsNum,sessionNum,sessionsNum*4 +1,trial-trialCount+1,trialsNum)
        if ratsType == 'LgA':
            loggingLgA(trial,action,inState,setpointS,out)    
...
def plotInternalState():
    ...
    tick_lbs = []
    for i in range ( 0 , 16 ):
        tick_lcs.append( i*15*15 ) 
        tick_lbs.append(i*15)
...
def plotEtimatedCocaineProbabilityFirstLgA():
    ...
    leg = fig1.legend((S0, S1), ('Cocaine state','NoCocaine state'), loc = (0.58,0.42))
    ...
    ax1.set_ylabel('Subjective cocaine probability')
    ax1.set_xlabel('Time (min)')
    ax1.set_title('')
...
def plotEtimatedCocaineProbabilityLgA():
    ...
    ax1 = fig1.add_subplot(111)
    S0 = ax1.plot(estimatedOutcomeUnderCocaLgA [ 0 : extinctionTrialsNum + (extinctionTrialsNum*4 + 4)*sessionsNum ]/50 , linewidth = 2.5 , color='black' )
    S1 = ax1.plot(estimatedOutcomeNoCocaLgA    [ 0 : extinctionTrialsNum + (extinctionTrialsNum*4 + 4)*sessionsNum ]/50 , linewidth = 1.5 , color='black' )
    ...
    pylab.yticks(pylab.arange(0, 1.1, 0.2))
    pylab.ylim((-0.2,1.2))
    pylab.xlim( ( -500 , extinctionTrialsNum + (extinctionTrialsNum*4 + 4)*sessionsNum + 500) )
    ...
    leg = fig1.legend((S0, S1), ('Cocaine state','NoCocaine state'), loc = (0.42,0.62))
    leg.draw_frame(False)      
...
def plotLeverPressPerDose():
    ...
    infusionPerSessionLgAFisrt = numpy.zeros( [5] , float)
    infusionPerSessionShAFisrt = numpy.zeros( [5] , float)
    ...
    for i in range(0,5):
        for j in range( extinctionTrialsNum + i*(extinctionTrialsNum+1) , extinctionTrialsNum + i*(extinctionTrialsNum+1) + extinctionTrialsNum ):
            ...
            infusionPerSessionShAFisrt[i] = infusionPerSessionShAFisrt[i] + activeLeverPressShA[j]
    ...
    S1 = ax1.plot(x,infusionPerSessionShAFisrt , '-o', ms=8, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )

    leg=fig1.legend((S0, S1), ('LgA','ShA'), loc = (0.18,0.68))
    leg.draw_frame(False)
...
def plotLeverPressPer5Min():
    ...
    infusionPerSessionLgAFirst = numpy.zeros( [5*9] , float)
    infusionPerSessionShAFirst = numpy.zeros( [5*9] , float)
    ...
    for i in range(0,5):
        for k in range(0,9):
            for j in range(  i*(extinctionTrialsNum+1) + k*75 , i*(extinctionTrialsNum+1) + k*75 + 75 ):
                infusionPerSessionLgAFirst[i*9 + k] = infusionPerSessionLgAFirst[i*9 + k] + activeLeverPressLgA[j]
                infusionPerSessionShAFirst[i*9 + k] = infusionPerSessionShAFirst[i*9 + k] + activeLeverPressShA[j]
    ...
    S1 = ax1.plot(x[0:9]  ,infusionPerSessionShAFirst[0:9]    , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
    S1 = ax1.plot(x[9:18] ,infusionPerSessionShAFirst[9:18]   , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
    S1 = ax1.plot(x[18:27],infusionPerSessionShAFirst[18:27]  , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
    S1 = ax1.plot(x[27:36],infusionPerSessionShAFirst[27:36]  , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
    S1 = ax1.plot(x[36:45],infusionPerSessionShAFirst[36:45]  , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='black',linewidth = 2 , color='black' )
    
    S0 = ax1.plot(x[0:9]  ,infusionPerSessionLgAFirst[0:9]    , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )
    S0 = ax1.plot(x[9:18] ,infusionPerSessionLgAFirst[9:18]   , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )
    S0 = ax1.plot(x[18:27],infusionPerSessionLgAFirst[18:27]  , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )
    S0 = ax1.plot(x[27:36],infusionPerSessionLgAFirst[27:36]  , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )
    S0 = ax1.plot(x[36:45],infusionPerSessionLgAFirst[36:45]  , '-o', ms=5, markeredgewidth =2, alpha=1, mfc='white',linewidth = 2 , color='black' )

    leg=fig1.legend((S0, S1), ('LgA','ShA'), loc = (0.14,0.23))
    leg.draw_frame(False)
    ...
    tick_lbs = []
    for i in range ( 0 , 16 ):
        tick_lcs.append( i*3  + 1 ) 
        tick_lbs.append( i*15 + 5 )
    pylab.xticks(tick_lcs, tick_lbs)
...
totalTrialsNum      = extinctionTrialsNum + (extinctionTrialsNum+1) * 5 * sessionsNum
...
for animal in range(0,animalsNum):
    ...
    initialSetpoint     = optimalInStateLowerBound
    initializeAnimal      (                               )

    extinction            ( 1, extinctionTrialsNum   ,'ShA')

    for session in range(0,sessionsNum):

        state[1] = initialInState
        
        noncontingentInfusion ( session*4+2 , 0                     ,'ShA')
        extinction            ( session*4+2 , extinctionTrialsNum   ,'ShA')
        noncontingentInfusion ( session*4+2 , cocaine               ,'ShA')
        extinction            ( session*4+2 , extinctionTrialsNum   ,'ShA')
        noncontingentInfusion ( session*4+3 , cocaine*2             ,'ShA')
        extinction            ( session*4+3 , extinctionTrialsNum   ,'ShA')
        noncontingentInfusion ( session*4+4 , cocaine*4             ,'ShA')
        extinction            ( session*4+4 , extinctionTrialsNum   ,'ShA')    
        noncontingentInfusion ( session*4+5 , cocaine*8             ,'ShA')
        extinction            ( session*4+5 , extinctionTrialsNum   ,'ShA')    
    ...
    for session in range(0,sessionsNum):
        ...
        noncontingentInfusion ( session*4+2 , cocaine               ,'LgA')
        extinction            ( session*4+2 , extinctionTrialsNum   ,'LgA')
